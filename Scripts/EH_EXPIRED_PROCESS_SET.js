/*------------------------------------------------------------------------------------------------------/
| Program: EH_EXPIRED_PROCESS_SET.js  Trigger: Batch
| Client:
|
...
var configStdChoice = "EH_EXPIRE_CONFIG"; // the standard choice that contains the batch renewal configuration information
	...
	for (var i = 0; i < SetMemberArray.length; i++) {
		...
		if (feeList!=null && feeList.length > 0) {
			...
			for(var fee in feeList){
				var feeDetails = fee.split(",");
				
				var version = feeDetails[0];
				var feeCode = feeDetails[1];
				var feeSchedule = feeDetails[2];
				...
				var feeSeq = null;
				assessFeeResult = aa.finance.createFeeItem(renewalCapId, feeSchedule, version, feeCode, paymentPeriod, feeQty);
				if (assessFeeResult.getSuccess()) {
					...
					if (feeInvoice == "Y"){ 
						...
						if (invoiceResult_L.getSuccess())
							logMessage("Invoicing assessed fee items" + renewalCapId + " is successful.");
						else
							logDebug("**ERROR: Invoicing the fee items assessed" + renewalCapId + " was not successful.  Reason: " + invoiceResult_L.getErrorMessage());
					}
				} else {
