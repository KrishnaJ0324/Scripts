/*------------------------------------------------------------------------------------------------------/
| Program: EH_RENEWAL_PROCESS_SET.js  Trigger: Batch
| Client:
|
| Version 1.0 - Base Version. 04/19/2016
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
var configStdChoice = "EH_RENEWAL_CONFIG"; // the standard choice that contains the batch renewal configuration information
...
if (emailAddress.length)
	aa.sendMail("auto_sender@missionsky.com", emailAddress, "", ScriptName + " Results", emailText);
...
function mainProcess() {
	...
	for (var i = 0; i < SetMemberArray.length; i++) {
		...
		if (sendEmailToContactTypes.length > 0 && emailTemplate.length > 0) {
			...
			for (thisCon in conArray) {
				...
				if (conEmail) {
					...
					addParameter(emailParameters, "$$capType$$", appTypeString);
		...
		// add fees
		/*var feeList = getFeeList(appTypeString);
		var feeSeq_L = new Array(); // invoicing fee for CAP in args
		var paymentPeriod_L = new Array(); // invoicing pay periods for CAP in args
		if (feeList!=null && feeList.length > 0) {
			//each fee item should be in this format: fVersion, fcode, fsched, fperiod, fqty, finvoice 
			for(var fee in feeList){
				var feeDetails = feeList[fee].split(",");
				
				var version = feeDetails[0];
				var feeSchedule = feeDetails[1];
				var feeCode = feeDetails[2];
				var paymentPeriod = feeDetails[3];
				var feeQty = feeDetails[4];
				var feeInvoice = feeDetails[5];
				
				if(isEmpty(paymentPeriod))
					paymentPeriod = "FINAL";
				if(isEmpty(feeQty))
					feeQty = 1;
				if(isEmpty(feeInvoice))
					feeInvoice = "N";
				
				var feeSeq = null;
				if(isEmpty(feeSchedule) || isEmpty(version) || isEmpty(feeCode)){
					logDebug("feeSchedule, feeCode or version is not set up for "+appTypeString + " in standard choice: EH_RENEWAL_FEE");
				}else{
					assessFeeResult = aa.finance.createFeeItem(capId, feeSchedule.trim(), version.trim(), feeCode.trim(), paymentPeriod.trim(), feeQty);
					if (assessFeeResult.getSuccess()) {
						...
						if (feeInvoice == "Y"){ 
							...
							paymentPeriod_L.push(paymentPeriod);
							var invoiceResult_L = aa.finance.createInvoice(capId, feeSeq_L, paymentPeriod_L);
							if (invoiceResult_L.getSuccess())
								logMessage("Invoicing assessed fee items" + capId + " is successful.");
							else
								logDebug("**ERROR: Invoicing the fee items assessed" + altId + " was not successful.  Reason: " + invoiceResult_L.getErrorMessage());
						}
						logDebug("Successfully added fee "+feeDetails.toString()+" to cap:"+altId+" fee sequence number is "+feeSeq);
					} else {
						logDebug("**ERROR: assessing fee (" + feeCode + "): " + assessFeeResult.getErrorMessage());
					}
				}
				
			}
		} else{
			logDebug("failed to get fee List for "+appTypeString);
			continue;
		}*/
	}
	...
	aa.env.setValue("ScriptReturnMessage", "Update Set successful - Permit Renewal Process Script");
...
function getFeeList(appTypeStr){
	//appTypeStr, e.g. "EH/Food/Commissary/Permit"
	var feeList = lookup("EH_RENEWAL_FEE",appTypeStr);
	
	if(feeList!=undefined && feeList!=null){
		var feeArray = new Array();
		//remove empty elements
		feeArray = feeList.split(";").filter(function(e){return e}); 
		return feeArray;
	}
...
function isEmpty(v) {
	if (v === undefined || typeof(v) == 'undefined' || v == null || (typeof(v) == 'string' && v.length ==0) || (v + '' == '')) {
		return true;
	}
	return false;
}
