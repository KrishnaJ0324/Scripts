/*------------------------------------------------------------------------------------------------------/
| Program: EH_EXPIRED_BATCH_CREATE_SET.js  Trigger: Batch
| Client:
|
| Version 1.0 - Base Version. 04/19/2016
| Version 1.5 - Modify the fromDate and toDate base on lookAheadDays and daySpan. -- 07/15/2016
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
//aa.env.setValue("configStdChoice","LIC_RENEWAL_CONFIG");
...
if (!configStdChoice.length) 
	configStdChoice = "EH_EXPIRE_CONFIG";
...
if (fromDate.length < 10) { // no "from" date, assume today + number of days to look ahead
	...
	var timeDiff = parseInt(lookAheadDays) - parseInt(daySpan);
	// aa.print(timeDiff)
	if( timeDiff < 0  ){
		fromDate = dateAdd(null,timeDiff)
	}
}	
...
if (emailAddress.length)
	aa.sendMail("auto_sender@missionsky.com", emailAddress, "", batchJobName + " Results", emailText);
...
function mainProcess(){
	...
	for (thisExp in myExp){  // for each b1expiration (effectively, each license app)
		...
		if (setPrefix != "" && capCount == 1){
			...
			setName = setPrefix.substr(0,20) + "-" + mm + dd + yy + mm + hh + mi;
