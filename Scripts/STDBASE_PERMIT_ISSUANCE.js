...
Description : JSON Example:custom
...
try {
	...
	isConfigurableScript(settingsArray, scriptSuffix);
        showDebug = 1;
        logDebug(showDebug);
        logDebug("Initiating process.....");
        logDebug(settingsArray)
        fetchData({from:"STDBASE_PERMIT_ISSUANCE",data:settingsArray})
		...
		for ( var t in rules.action.updateTask) {
			...
			if (currentTask != null && currentTask != "") {
				...
				var updateResult = aa.workflow.handleDisposition(currentTask.getTaskItem(), capId);
                                logDebug("coming inside the condition");
			}
		}
	}//updateTask
